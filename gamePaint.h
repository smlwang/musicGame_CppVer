...
struct GamePainter {
	...
	void PaintCombo(HDC hdc, Game& game, long now) {
		...
		auto font = CreateFont(siz * 3, siz * 2, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS,
	...
	void PaintScore(HDC hdc, Game& game) {
		...
		auto font = CreateFont(Info.score.size * 1.5, Info.score.size, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS,
			CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Impact"));
		SelectObject(hdc, font);
