...
struct GamePainter {
    char buffer[512];
    void PaintGame(HDC hdc, Game &game) {
    ...
    void PaintNote(HDC hdc, Game &game) {
        long now = game.time();
        for (int i = 0; i < 4; i++) {
            ...
            while (st != ed) {
                ...
                long perfect = Info.baseline.y - (st->st - now) * Info.speed;
                ...
                if (st->kind == Info.note.click) {
                    ...
                    Rectangle(hdc, left, perfect - Info.note.perfectHigh,
                              left + Info.track.width,
                              perfect + Info.note.perfectHigh);
                    DeleteObject(brush);
                } else {
                    long top = Info.baseline.y - (st->ed - now) * Info.speed;
                    auto brush = CreateSolidBrush(color);
                    SelectObject(hdc, brush);
                    Rectangle(hdc, left + Info.note.adjust, top,
                              left + Info.track.width - Info.note.adjust,
                              bottom);
                    if (!st->preCheck) {
                        Rectangle(hdc, left, bottom - Info.note.bhigh,
                                  left + Info.track.width,
                                  bottom + Info.note.adjust);
                    }
                    DeleteObject(brush);
                }
                ++st;
    ...
    int cal_size(long dif) {
        return 1 + (Info.judge.square - dif * dif) / 6000;
    }
    ...
    void PaintCombo(HDC hdc, Game &game, long now) {
        if (game.stat.code == game.stat.end ||
            game.combo < Info.minComboToShow) {
            return;
        }
        int siz = Info.combo.size;
        if (now - game.pcombo.time >= Info.combo.anime_time) {
            game.pcombo.sizeLevel = min(10, game.combo + 1);
            game.pcombo.time = now;
        }
        siz += game.pcombo.sizeLevel * 1.5;
        int oldmode = SetBkMode(hdc, TRANSPARENT);
        auto old = SetTextColor(hdc, Info.combo.color);
        auto font =
            CreateFont(siz * 3, siz * 2, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE,
                       DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS,
                       CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Impact"));
        SelectObject(hdc, font);
        sprintf_s(combobuffer, "%d", game.combo);
        RECT rect = Info.combo.rect;
        DrawText(hdc, combobuffer, -1, &rect, DT_CENTER | DT_VCENTER);
        SetTextColor(hdc, old);
        SetBkMode(hdc, oldmode);
        DeleteObject(font);
    }
    void PaintJudgeStat(HDC hdc, PaintEvent &pe, long now) {
        ...
        auto font =
            CreateFont(siz * 5, siz * 3, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE,
                       DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS,
                       CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Impact"));
        SelectObject(hdc, font);
    ...
    void PaintTrackHitInfo(HDC hdc, Game &game, long now) {
        for (int i = 0; i < 4; i++) {
            ...
            if (track.pressed()) {
                ...
                POINT p[3] = {
                    {mid, y + 10}, {mid - 30, y + 40}, {mid + 30, y + 40}};
    ...
    void PaintGameProccess(HDC hdc, Game &game) {
        int per = game.time() * Info.timeLine.length / game.totlen;
        auto brush = CreateSolidBrush(Info.timeLine.base_color);

        SelectObject(hdc, brush);
        Rectangle(hdc, Info.timeLine.x, Info.timeLine.y,
                  Info.timeLine.x + Info.timeLine.length,
    ...
    void PaintInfo(HDC hdc, Game &game) {
        long now = game.time();
    ...
    void PaintReplayInfo(HDC hdc) {
        ...
        auto font =
            CreateFont(28, 14, 0, 0, 100, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
        ...
        DrawText(hdc, info.c_str(), -1, &rect, DT_LEFT | DT_VCENTER);
    ...
    void PaintPause(HDC hdc) {
        ...
        RECT rect = minfo.helper.rect;
        DrawText(hdc,
                 "press 'q' to back to MainManu\npress 'p' to continue\npress "
                 "'r' to restart current song",
    ...
    void PaintScore(HDC hdc, Game &game) {
        ...
        auto font =
            CreateFont(Info.score.size * 1.5, Info.score.size, 0, 0,
                       FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                       OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS,
                       CLEARTYPE_QUALITY, VARIABLE_PITCH, TEXT("Impact"));
        SelectObject(hdc, font);
        sprintf_s(combobuffer, "%d", game.score);
    ...
    void PaintEnd(HDC hdc, Game &game) {
        ...
        font = CreateFont(Info.statCount.w, Info.statCount.h, 0, 0, 100, FALSE,
